...
abbrev: "Cookies: HTTP State Management Mechanism"
...
normative:
  ...
  RFC9110:
     display: HTTP
  USASCII:
    target: https://webstore.ansi.org/standards/incits/incits1986r2022
    title: "Information Systems - Coded Character Sets - 7-Bit Standard Code for Information Interchange (7-Bit ASCII)"
    seriesinfo:
      INCITS: 4-1986 (R2022)
    date: 2022
    author:
  ...
  DOM-DOCUMENT-COOKIE:
    ...
    ann: >
      Commit snapshot: [](https://html.spec.whatwg.org/commit-snapshots/b4b94b1db80aa55a143e1f74f6ad9d4a185f2333/#dom-doument-cookie){:brackets="angle"}.
  SAMESITE:
    target: https://html.spec.whatwg.org/#same-site
    title: HTML - same site
    date: false
    author:
      -
        org: WHATWG
    refcontent: WHATWG Living Standard
    ann: >
      Commit snapshot: [](https://html.spec.whatwg.org/commit-snapshots/b4b94b1db80aa55a143e1f74f6ad9d4a185f2333/#same-site){:brackets="angle"}.
...
informative:
  ...
  RFC9114:
  CSRF:
    target: https://dl.acm.org/doi/10.1145/1455770.1455782
    ...
    refcontent: "CCS '08: Proceedings of the 15th ACM conference on
  Computer and communications security, pp. 75-88"
  Aggarwal2010:
    author:
    -
      ins: G. Aggarwal
    ...
    date: 2010
    refcontent: 19th Usenix Security Symposium
    target: http://www.usenix.org/events/sec10/tech/full_papers/Aggarwal.pdf
  app-isolation:
    target: https://dl.acm.org/doi/10.1145/2046707.2046734
    ...
    refcontent: "CCS '11: Proceedings of the 18th ACM conference on
  Computer and communications security, pp. 227-238"
  FETCH:
    title: Fetch
    author:
    -
    ...
    refcontent: WHATWG Living Standard
    ann: >
      Commit snapshot: [](https://fetch.spec.whatwg.org/commit-snapshots/d9c7b6d31067a7eb13110950ef67db54c6362cfa/){:brackets="angle"}.
  HTML:
    title: HTML
    ...
    refcontent: WHATWG Living Standard
    ann: >
      Commit snapshot: [](https://html.spec.whatwg.org/commit-snapshots/b4b94b1db80aa55a143e1f74f6ad9d4a185f2333/){:brackets="angle"}.
  prerendering:
    target: https://developer.chrome.com/docs/web-platform/prerender-pages
  ...
  SERVICE-WORKERS:
    ...
    target: https://www.w3.org/TR/2026/CRD-service-workers-20260126/
    date: 2026-01-26
    author:
      -
    ...
    ann: >
      Latest version available at [](https://www.w3.org/TR/service-workers/){:brackets="angle"}.
...
{{RFC5234, Appendix B.1}}: ALPHA (letters), CR (carriage return), CRLF (CR LF), CTLs
...
"sandboxed origin browsing context flag", "shared worker", 
"top-level traversable", and "WorkerGlobalScope" are defined in
{{HTML}}.

"Service Workers" are defined in the {{SERVICE-WORKERS}} specification.

The term "origin", the mechanism of deriving an origin from a URI, and "the
...
For example, for `https://www.site.example`, the public suffix is `example`, and the
...
It could be intended as two cookies, a=b and d=e, or a single
...
characters that are not {{USASCII}}, no attempt should be made to interpret these octets
...
The upcoming two sections, Sections {{sane-profile}}{:format="counter"} and {{ua-requirements}}{:format="counter"}, discuss
...
An implementer should implement the requirements described in {{sane-profile}} whenever cookies are created and
...
that primarily produces cookies. Some potential examples include:
...
software framework and is later sent back to a server application that needs
...
An implementer should implement the requirements described in {{ua-requirements}} whenever cookies are primarily
...
or not that server is following best practices, user agents are advised to
...
#### Programming Languages and Software Frameworks {#languages-frameworks}
...
should strongly consider supporting both sets of requirements (i.e., the requirements in both Sections {{sane-profile}}{:format="counter"}
and {{ua-requirements}}{:format="counter"}) behind a compatibility mode toggle. This toggle should
default to the requirements in {{sane-profile}}.
...
accepted (e.g., "httponly", "Httponly", "hTTPoNLY").
...
Layer Security (TLS); see {{RFC8446}} and {{RFC9110}}).
...
that assert `SameSite=Lax` or `SameSite=Strict` cannot be set in responses to
...
Sections {{weak-confidentiality}}{:format="counter"} and {{weak-integrity}}{:format="counter"} of this document spell out some of the drawbacks of cookies'
...
Requests that are the result of a reload navigation triggered through a user
...
execution context with only a tangential relationship to the Document that
registered them.
...
That is, Set-Cookie header fields contained in responses with status codes that are not 100 level
(including those in responses with 400- and 500-level status codes)
...
servers' ({{server-name-prefixes}}) in that user agents MUST match the prefix string
case-insensitively.
...
to a user agent that checks prefixes case-sensitively, it will accept this cookie, and
the server would incorrectly believe the cookie is subject to the same guarantees
...
sends the following Set-Cookie header field for the site to a user agent that checks
...
The next time a user visits the site, the user agent will send both cookies:
...
To prevent these issues, user agents MUST match cookie name prefixes
case-insensitively.

Note: Cookies with different names are still considered separate by user agents. So
...
1.  If the set-cookie-string contains a %x00-08 / %x0A-1F / %x7F character
    (CTL characters excluding HTAB), then
...
top-level navigations that are triggered from a cross-site document context.
As discussed in {{top-level-navigations}}, this might or might not be compatible
with existing session management systems. In the interest of providing a
drop-in mechanism that mitigates the risk of cross-site request forgery (CSRF) attacks, developers may set
the SameSite attribute in a "Lax" enforcement mode that carves out an
exception that sends same-site cookies along with cross-site requests if and
only if they are top-level navigations that use a "safe" (in the {{RFC9110}}
...
these cases, a request's "safeness" is determined based on the method of the
current redirect hop.)

Lax enforcement provides reasonable defense in depth against CSRF attacks that
rely on unsafe HTTP methods (like `POST`), but it does not offer a robust defense
...
compatibility mode, user agents that apply "Lax-allowing-unsafe" enforcement
SHOULD restrict the enforcement to cookies that were created recently.
...
5.  Create a new cookie with name cookie-name and value cookie-value. Set the
...
18. If the cookie's same-site-flag is not "None":
    ...
    2.  If the cookie was received from a same-site request (as defined in
        {{same-site-requests}}), skip the remaining sub-steps and continue
        processing the cookie.

    3.  If the cookie was received from a request that is navigating a
        ...
        sub-steps and continue processing the cookie.
...
2.  Cookies whose secure-only-flag is false and that share a domain field
...
of a cookie-string. A "retrieval" is any event that requires generating a
...
3. Let cookie-list be the set of cookies from the cookie store that meets all
   ...
   * Either:
     ...
     *  The cookie's domain is not a public suffix, and the user agent is configured
     ...
     creation if it had been created now. (See Step 9 in {{storage-model}}.)
...
6. Serialize the cookie-list into a cookie-string by processing each cookie
   ...
   1.  If the cookie's name is not empty, output the cookie's name followed by
       the %x3D ("=") character.

   2.  If the cookie's value is not empty, output the cookie's value.
...
User agents MAY limit the maximum number of cookies they store, and they may evict
...
the stored cookies, due to the length limits that  MUST be enforced in
{{set-cookie}}.

Servers SHOULD use as few and as small cookies as possible to avoid reaching
these implementation limits, to minimize network bandwidth due to the
...
can happen on a single site, but it is most problematic when activity is tracked across different,
seemingly unconnected websites to build a user profile.
...
It is too early to declare consensus on which specific mechanism(s) should be used to mitigate cookies' privacy impact; user agents' ongoing changes to how they are handled are best characterized as experiments that
...
scripts, images, stylesheets, or frames) that is obtained from a different server than the one that
hosts the primary resource (usually, the webpage that the user is viewing). Third-party cookies
...
Set-Cookie header fields and refusing to send third-party Cookie header fields. Some user agents partition
cookies based upon the first-party context, so that different cookies are sent depending on the site being browsed.
Some user agents block cookies based upon user agent cookie policy and/or user controls.
...
cookies across sessions. When configured as such, user agents MUST treat all
...
Although this security concern goes by a number of names (e.g., CSRF
or confused deputy), the issue stems from cookies being a form of
...
from its user agent to the victim's user agent. Second, the victim uses
...
Third, the attacker uses the session identifier to interact with the server
...
attacker can replay a cookie it received from the authentic site.example
...
SameSite cookies offer a robust defense against CSRF attacks when deployed in
...
depth against CSRF attacks, but it has the potential to confuse users unless sites'
...
relies on not one, but two cookies: one conceptually granting "read" access and
...
that are required in these situations should be marked with `SameSite=None`
...
decision. Moreover, a number of side channels exist that could allow a server
...
of other reload navigations, which are always same site if they are top level, since the
...
The "Hypertext Transfer Protocol (HTTP) Field Name Registry" (see {{HttpFieldNameRegistry}}) has been
...
Reference:
: RFC 10025 ({{set-cookie}})

## "Cookie Attributes" Registry

IANA has created the "Cookie Attributes" registry, which defines the name space of
attributes used to control cookies' behavior. The registry is maintained in a new
registry group called "Hypertext Transfer Protocol (HTTP) Cookie Attributes" at
[](https://www.iana.org/assignments/cookie-attribute-names){:brackets="angle"}.
...
New registrations happen on an "RFC Required" basis (see {{Section 4.7 of
...
The "Cookie Attributes" registry has been created with the registrations below:

| Name     | Reference                               |
|----------:+----------------------------------------|
| Domain   | {{attribute-domain}} of RFC 10025   |
| Expires  | {{attribute-expires}} of RFC 10025  |
| HttpOnly | {{attribute-httponly}} of RFC 10025 |
| Max-Age  | {{attribute-max-age}} of RFC 10025  |
| Path     | {{attribute-path}} of RFC 10025     |
| SameSite | {{attribute-samesite}} of RFC 10025 |
| Secure   | {{attribute-secure}} of RFC 10025   |
...
(Sections {{same-site-requests}}{:format="counter"} and {{attribute-samesite}}{:format="counter"}).

*  Introduces cookie prefixes and prohibits nameless cookies from setting a
value that would mimic a cookie prefix (Sections {{server-name-prefixes}}{:format="counter"} and
{{storage-model}}{:format="counter"}).
...
*  Limits maximum values for max-age and expire (Sections {{ua-attribute-expires}}{:format="counter"} and {{ua-attribute-max-age}}{:format="counter"}).
...
* Improves cookie syntax
  ...
  *  Handles control characters explicitly (Sections {{set-cookie}}{:format="counter"} and {{storage-model}}{:format="counter"}).
...
*  Addresses errata 3444 by updating the path-value and extension-av grammar,
errata 4148 by updating the day-of-month, year, and time grammar, and errata
3663 by adding the requested note (see Sections {{sane-set-cookie}}{:format="counter"} and {{cookie-path}}{:format="counter"}).

# Acknowledgements
{:numbered="false"}
{{RFC6265}} was written by {{{Adam Barth}}}. This document is an update of {{RFC6265}},
...
significant contributions improving this document.
